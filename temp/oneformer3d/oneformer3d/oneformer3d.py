...
from .ptv2_encoder import PTV2Encoder
...
class ScanNetOneFormer3D(ScanNetOneFormer3DMixin, Base3DDetector):
    ...
    def extract_feat(self, x, superpoints, inverse_mapping, batch_offsets):
        ...
        x = self.input_conv(x)
        # [N, 32]
        # print('####after input_conv:', x.features.shape)
        x, _ = self.unet(x)
        # [N, 32]
        # print('####after unet:', x.features.shape)
        x = self.output_layer(x)
        # [N, 32]
        # print('####after output_layer:', x.features.shape)
...
@MODELS.register_module()
class ScanNetOneFormer3DWithPointNet2(ScanNetOneFormer3DMixin, Base3DDetector):
    r"""OneFormer3D for ScanNet dataset with PointNet2 as feature extractor.
    ...
    def __init__(self,
                 ...
                 data_preprocessor=None,
                 init_cfg=None):
        super(ScanNetOneFormer3DWithPointNet2, self).__init__(
            data_preprocessor=data_preprocessor, init_cfg=init_cfg)
        # Initialize PointNet2Encoder instead of UNet
        self.pointnet2 = PointNet2Encoder(in_dim=in_channels, out_dim=num_channels)
        self.decoder = MODELS.build(decoder)
        self.criterion = MODELS.build(criterion)
        self.voxel_size = voxel_size
        self.num_classes = num_classes
    ...
    def extract_feat(self, x, superpoints, inverse_mapping, batch_offsets):
        """Extract features from sparse tensor using PointNet2."""
        # [N, 6] : 6->xyzrgb
        # print('####input:', x.features.shape)
        x = self.pointnet2(x)
        # [N, 32]
        # print('####after pointnet2:', x.features.shape)
        x = self.output_layer(x)
        # [N, 32]
        # print('####after output_layer:', x.features.shape)
        x = scatter_mean(x[inverse_mapping], superpoints, dim=0)
        out = []
        for i in range(len(batch_offsets) - 1):
            out.append(x[batch_offsets[i]: batch_offsets[i + 1]])
        return out
...
@MODELS.register_module()
class ScanNetOneFormer3DWithPTV2Encoder(ScanNetOneFormer3DMixin, Base3DDetector):
    r"""OneFormer3D for ScanNet dataset with the encoder of Point Transformer as feature extractor.
    ...
    def __init__(self,
                 ...
                 data_preprocessor=None,
                 init_cfg=None):
        super(ScanNetOneFormer3DWithPTV2Encoder, self).__init__(
            data_preprocessor=data_preprocessor, init_cfg=init_cfg)
        # Initialize PointNet2Encoder instead of UNet
        self.pt_encoder = PTV2Encoder(
...
@MODELS.register_module()
class ScanNetOneFormer3DWithPTV3Encoder(ScanNetOneFormer3DMixin, Base3DDetector):
    r"""OneFormer3D for ScanNet dataset with the encoder of Point Transformer as feature extractor.
    ...
    def __init__(self,
                 ...
                 data_preprocessor=None,
                 init_cfg=None):
        super(ScanNetOneFormer3DWithPTV3Encoder, self).__init__(
            data_preprocessor=data_preprocessor, init_cfg=init_cfg)
        # Initialize PointNet2Encoder instead of UNet
        self.pt_encoder = PTV3Encoder(
            in_channels=in_channels,
            out_channels=num_channels)
        self.decoder = MODELS.build(decoder)
        self.criterion = MODELS.build(criterion)
